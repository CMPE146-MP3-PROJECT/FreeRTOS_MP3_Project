...
void spi0__pin_init(void) {
  ...
  gpio__construct_with_function(GPIO__PORT_0, 17, GPIO__FUNCTION_2); // enable SSP0_MOSI

  LPC_IOCON->P0_18 &= ~GPIO_pin_function_reset_mask;
  gpio__construct_with_function(GPIO__PORT_0, 18, GPIO__FUNCTION_2); // enable SSP0_MISO
}
...
void adafruit_cs(void) {
  gpiox__set_low(adafruit_cs_ds_pin); // activate flash
}
void adafruit_ds(void) {
  gpiox__set_high(adafruit_cs_ds_pin); // set CS to high to deactivate flash
}
...
void spi0__mp3_init(uint32_t max_spi_clock_mhz) {
  ...
  uint8_t spi_MS_mode_bit_mask =
      (1
       << 2); //(1 << 2); 0 = master mode; 1 = slave mode; this bit can only be written when SPI controller is disabled
  ...
  if (devider_prescalar > 254) {
    devider_prescalar = 254U;
  }
...
uint8_t spi0__mp3_exchange_byte(uint8_t data_out) {
  ...
  uint8_t spi_channel_busy_indication = (1 << 4); //(1 << ); 1 = busy, 0 = free.
  LPC_SSP0->DR = data_out;                        // 给DR写数据
