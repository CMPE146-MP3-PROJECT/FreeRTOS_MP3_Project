...
void spi1__pin_init(void) {
  ...
  gpio__construct_with_function(GPIO__PORT_0, 9, GPIO__FUNCTION_2); // enable SSP2_MOSI
...
void spi1__OLED_init(uint32_t max_spi_clock_mhz) {
  ...
  LPC_SSP1->CR1 |= spi_controller_bit_mask; // enable spi0 controller
  ...
  // OLED_print_string(1, 0, 0, " I'm the King! ", 16);
  // OLED_print_string(4, 0, 0, "     Press *   ", 16);
  // OLED_print_string(5, 0, 0, "to enter system", 16);
}

uint8_t spi1__OLED_exchange_byte(uint8_t data_out) {
  uint8_t spi_channel_busy_indication = (1 << 4); //(1 << 4); 1 = busy, 0 = free.
  LPC_SSP1->DR = (uint16_t)data_out;
...
void OLED_cs(void) { gpiox__set_low(OLED_CS_pin); }

void OLED_ds(void) { gpiox__set_high(OLED_CS_pin); }
