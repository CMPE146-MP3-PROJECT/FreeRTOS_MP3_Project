...
class UnityTestRunnerGenerator
  ...
  def create_main(output, filename, tests, used_mocks)
    ...
    if @options[:has_suite_teardown]
      ...
      output.puts("  int result = UnityEnd();")
      output.puts("  puts(\"\");")
      output.puts("  puts(\"\");")
      output.puts("  printf(\"--- \");")
      output.puts("  if (result == 0)")
      output.puts("    printf(\"PASSED\");")
      output.puts("  else")
      output.puts("    printf(\"FAILED\");")
      output.puts("  printf(\" UNIT TEST: [\");")
      output.puts("  printf(\"#{filename.gsub(/\\/, '\\\\\\')}\");")
      output.puts("  printf(\"] ---\\n\");")
      output.puts("  puts(\"\");")
      output.puts("  puts(\"\");")
